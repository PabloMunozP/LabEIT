    ...
    if request.method == 'POST':
        ...
        query = '''
            ...
            FROM
                ...
                Etiqueta, Etiqueta_equipo
            WHERE
                Equipo.id = Etiqueta_equipo.id_equipo AND Etiqueta.id = Etiqueta_equipo.id_etiqueta AND (Equipo.modelo LIKE '%s')
            GROUP BY Equipo.codigo'''
        cursor.execute(query,(busqueda_entrada))
        equipos = cursor.fetchall()
        return render_template('/busqueda-solicitudes/busqueda2.html',equipos=equipos)
